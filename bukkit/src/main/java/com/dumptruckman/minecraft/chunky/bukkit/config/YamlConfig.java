...
public class YamlConfig extends AbstractYamlConfig implements ChunkyConfig {
    ...
    private static final SimpleConfigEntry<Null> SETTINGS = new EntryBuilder<Null>(Null.class, "settings")
            .comment("# Settings").build();

    public YamlConfig(BukkitPlugin plugin) throws IOException {
        super(plugin, true, true, new File(plugin.getDataFolder(), "config.yml"), ChunkyConfig.class);
    }
